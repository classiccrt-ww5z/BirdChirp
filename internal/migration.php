...
<?php if ($output): ?>
  <div class="alert success"><?= $output ?></div>
<?php endif; ?>

<p>Reads <code>database/schema.sql</code> and compares it against the current database. Any missing tables or columns will be created automatically. Also creates the <code>admin_log</code> table.</p>
<p>Run this after updating schema.sql or deploying new features that need database changes.</p>

<form method="post" style="margin-top:16px;">
  <input type="hidden" name="csrf_token" value="<?= generateCSRF() ?>">
  <button type="submit" name="run_migration" class="btn primary" onclick="return confirm('Run migration? This will add any missing tables/columns.');">Run Migration</button>
</form>
