...
if (isset($_GET['delete'])) {
    if (!verifyCSRF($_GET['csrf'] ?? '')) { setMessage("error","CSRF failed."); header("Location: posts.php"); exit; }
    ...
    header("Location: posts.php?page=$page".($search?"&search=".urlencode($search):"")."&filter=$filter"); exit;
}
...
<form method="get" class="form-inline" style="margin-bottom:10px;">
  ...
  <?php if($search): ?> <a href="posts.php" class="btn btn-link btn-sm">Clear</a><?php endif; ?>
  <input type="hidden" name="filter" value="<?=e($filter)?>">
</form>

<ul class="nav nav-pills" style="margin-bottom:10px;">
  <li class="<?=$filter=='all'?'active':''?>"><a href="posts.php?filter=all<?=$search?'&search='.urlencode($search):''?>">All</a></li>
  <li class="<?=$filter=='images'?'active':''?>"><a href="posts.php?filter=images<?=$search?'&search='.urlencode($search):''?>">Images</a></li>
  <li class="<?=$filter=='videos'?'active':''?>"><a href="posts.php?filter=videos<?=$search?'&search='.urlencode($search):''?>">Videos</a></li>
  <li class="<?=$filter=='text'?'active':''?>"><a href="posts.php?filter=text<?=$search?'&search='.urlencode($search):''?>">Text</a></li>
</ul>
...
<tr>
  <td>#<?=$p['id']?></td>
  <td><a href="users.php?user_id=<?=$p['user_id']?>"><b><?=e($p['display_name']?:$p['username'])?></b></a></td>
  ...
  <td><a href="?delete=<?=$p['id']?>&csrf=<?=$adminCsrf?>&page=<?=$page?>&filter=<?=$filter?><?=$search?'&search='.urlencode($search):''?>" class="btn btn-danger btn-xs" onclick="return confirm('Delete?')">Delete</a></td>
</tr>
<?php endforeach; else: ?>
...
<?php if($totalPages>1): ?>
<ul class="pager">
  <?php if($page>1): ?><li><a href="?page=<?=$page-1?>&filter=<?=$filter?><?=$search?'&search='.urlencode($search):''?>">Prev</a></li><?php endif; ?>
  <li class="disabled"><a>Page <?=$page?> / <?=$totalPages?></a></li>
  <?php if($page<$totalPages): ?><li><a href="?page=<?=$page+1?>&filter=<?=$filter?><?=$search?'&search='.urlencode($search):''?>">Next</a></li><?php endif; ?>
</ul>
<?php endif; ?>
