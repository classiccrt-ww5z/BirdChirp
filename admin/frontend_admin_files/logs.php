...
<ul class="nav nav-pills" style="margin-bottom:10px;">
  <li class="<?=!$actionFilter?'active':''?>"><a href="index.php?page=logs" onclick="return loadPage('logs')">All</a></li>
  <?php foreach($actions as $a):?> <li class="<?=$actionFilter===$a?'active':''?>"><a href="index.php?page=logs&action=<?=urlencode($a)?>" onclick="return loadPage('logs','action=<?=urlencode($a)?>')"><?=e($a)?></a></li><?php endforeach;?>
</ul>
...
<?php if($totalPages>1):?>
<ul class="pager">
  <?php if($page>1):?><li><a href="index.php?page=logs&page=<?=$page-1?><?=$actionFilter?'&action='.urlencode($actionFilter):''?>" onclick="return loadPage('logs','page=<?=$page-1?><?=$actionFilter?'&action='.urlencode($actionFilter):''?>')">Prev</a></li><?php endif;?>
  <li class="disabled"><a>Page <?=$page?> / <?=$totalPages?></a></li>
  <?php if($page<$totalPages):?><li><a href="index.php?page=logs&page=<?=$page+1?><?=$actionFilter?'&action='.urlencode($actionFilter):''?>" onclick="return loadPage('logs','page=<?=$page+1?><?=$actionFilter?'&action='.urlencode($actionFilter):''?>')">Next</a></li><?php endif;?>
</ul>
<?php endif;?>
