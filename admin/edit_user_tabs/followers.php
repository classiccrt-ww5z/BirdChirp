<?php $s=$pdo->prepare("SELECT u.* FROM follows f JOIN users u ON f.follower_id=u.id WHERE f.following_id=? ORDER BY f.id DESC");$s->execute([$selectedUser['id']]);$fl=$s->fetchAll();?>
